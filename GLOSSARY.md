...
- **`@SpringBootApplication`** — a meta-annotation bundling three separate ones: `@Configuration` (this class can define beans), `@EnableAutoConfiguration` (inspect the classpath and wire sensible-default beans for whatever's found there), and `@ComponentScan` (walk this package and everything under it, registering any annotated class as a bean). The one line that turns a `main` method into a full Spring application. _(T021, `DiscoveryServiceApplication.java`)_
- **`@Configuration`** — marks a class as a source of bean definitions; without it, any `@Bean` methods inside the class are never invoked by Spring even if the class itself is otherwise on the classpath. _(T021, `RedisConfig.java`, `ExceptionHandlingConfig.java`)_
- **`@Bean`** — marks a method whose return value Spring should register as a managed singleton, for cases where a plain `@Component` won't do (constructing a third-party class, or needing custom setup logic rather than a bare constructor call). _(T021, `RedisConfig.java`)_
- **`@Import(SomeClass.class)`** — manually registers one specific class as a bean, bypassing component-scan entirely; used to pull a single class in from outside the app's own scanned package tree without widening the scan to cover that class's whole package (and everything else that package might add later). _(T021, `ExceptionHandlingConfig.java`)_
- **Maven `<parent>` (`spring-boot-starter-parent`)** — inherits a shared dependency-management BOM (Bill of Materials) that pins compatible versions for every `spring-boot-starter-*` artifact; a child `pom.xml` declares a dependency with no `<version>` tag at all and gets the parent's chosen version, so five sibling services can never silently drift onto mismatched, untested version combinations. _(T021, `pom.xml`)_
- **`spring.threads.virtual.enabled: true`** — switches Spring MVC's request-handling threads from platform (OS) threads to Java 21 virtual threads, which unmount from their carrier thread while blocked on I/O instead of holding it for the whole wait. _(T021, `application.yml`)_
- **YAML `---` document separator (Spring profiles)** — splits one `application.yml` file into multiple documents, each optionally scoped to a profile via `spring.config.activate.on-profile`; only the base document plus whichever one profile is active (`SPRING_PROFILES_ACTIVE`) gets merged in at startup. _(T021, `application.yml`)_
...
- **`IOptions<TOptions>`** (`Microsoft.Extensions.Options`) — wraps a typed settings class bound from a configuration section, so a class asks for e.g. `IOptions<JwtIssuerOptions>` and gets back one already-populated object with named properties, instead of doing string-keyed `IConfiguration["Jwt:Issuer"]` lookups with no compile-time safety. _(T016, `JwtIssuerOptions.cs`, `JwtIssuer.cs`)_
...
- **`[ApiController]`** — marks a class as an API controller: enables automatic `400` responses on invalid model binding/validation, and infers `[FromBody]` for complex-type action parameters without writing it explicitly. _(T016, `AuthController.cs`)_
...
- **`RSA.Create(2048)` / `RSA.ImportFromPem(pem)`** — the two ways this codebase obtains an RSA keypair: generate a brand-new one in memory (dev-only fallback, lost on restart), or load one from an existing PEM-encoded string (the persisted, production path). Both produce one `RSA` object holding the full keypair — private and public halves together, never generated separately. _(T016, `JwtIssuer.cs`)_
- **`rsa.ExportParameters(includePrivateParameters: false)`** — exports only the public half (modulus + exponent) of an RSA keypair; the `false` here is the entire security boundary that keeps the private key from ever leaving this one class. _(T016, `JwtIssuer.cs`)_
- **`Base64UrlEncoder.Encode(...)`** (`Microsoft.IdentityModel.Tokens`) — base64url encoding (URL-safe alphabet, no padding), which is what the JWKS spec (RFC 7517) requires for a key's `n`/`e` fields — plain base64 would include characters (`+`, `/`, `=`) that aren't safe unescaped in a URL or JSON key. _(T016, `JwtIssuer.cs`)_
- **`new JwtSecurityToken(...)` + `JwtSecurityTokenHandler().WriteToken(token)`** — the signing/issuing side of the same library T012 already used for validation; constructing a `JwtSecurityToken` with claims + a signing key and calling `WriteToken` is what actually produces the compact JWT string handed back to a client. _(T016, `JwtIssuer.cs`)_
- **`SigningCredentials(key, SecurityAlgorithms.RsaSha256)`** — pairs a key with an explicit algorithm name for signing; pinning the algorithm here (rather than letting it be inferred from the key type) is the issuing-side half of the same "algorithm confusion" defense T012's `ValidAlgorithms` already enforces on the verifying side. _(T016, `JwtIssuer.cs`)_
- **`CryptographicOperations.FixedTimeEquals(byte[], byte[])`** — compares two byte arrays in constant time (always the same number of operations regardless of where they first differ), unlike `==`/`Equals`, which returns as soon as a mismatch is found and therefore leaks how many leading bytes matched via response timing. The standard .NET primitive for comparing secrets/hashes. _(T017, `OtpService.cs`)_

## Global Exception Handling (.NET)

- **`builder.Services.AddExceptionHandler<THandler>()`** — registers a class implementing `IExceptionHandler` as the app's catch-all for unhandled exceptions; paired with `app.UseExceptionHandler()` (no options object) so no framework fallback (RFC 7807 ProblemDetails) ever runs — this handler is the only thing that writes an error response. _(T017, `Program.cs`, `GlobalExceptionHandler.cs`)_
- **`IExceptionHandler.TryHandleAsync(...) : ValueTask<bool>`** — returning `true` tells the exception-handling middleware this exception has been fully handled and no further handler/fallback should also try to write a response; returning `false` lets the next registered handler (or the framework default) take over instead. _(T017, `GlobalExceptionHandler.cs`)_
